...
class StagingService {
  ...
  furnishRoom(room) {
    ...
    switch (room.type) {
      case 'master_bedroom': {
        // King Bed centered on top/rear wall
        const bedW = 6.5;
        const bedL = 6.5;
        ...
        // Flanking nightstands if width allows
        if (rw >= 11) {
        ...
        // Wardrobe along left or right wall
        if (rh >= 12) {
      ...
      case 'living': {
        // Living Lounge Zone
        if (rw >= 14 && rh >= 12) {
          // L-Shape Sectional or 3-seater
          const sofa = createItem('sofa_lshape', 1.0, 1.5, 0);
          if (sofa) furniture.push(sofa);

          // Central Coffee Table
          const table = createItem('coffee_table', 3.5, 4.0, 0);
          if (table) furniture.push(table);

          // TV Console opposite on right wall
          const tv = createItem('tv_unit', rw - 2.0, 2.0, 90);
          if (tv) furniture.push(tv);

          // Dining Table in opposite half of living hall
          if (rh >= 16) {
          ...
          const table = createItem('coffee_table', Math.max(1, (rw - 3.5) / 2), 4.5, 0);
          if (table) furniture.push(table);
